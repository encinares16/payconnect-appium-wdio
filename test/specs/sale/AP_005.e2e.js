...
describe(testCase.title.sale, () => {
    ...
    let testScenario = {
       AP_005: 'AP_005: Accept Payment - Visa card transaction (Tap, Debit Checking Account) should be processed successfully.',
    }

    it(testScenario.AP_005, async () => {
        addMetadata(metadata.AP_005)
        addBehaviors(behaviorsData.epic, behaviorsData.features, behaviorsData.story);
        helpers.testInfoLogger(testCase.title.auth, testScenario.AP_005, metadata.AP_005.description)
        ...
        try {
            ...
            await numberedSteps.start("Process a sale transaction.", async () => {
                await saleFlow(15000,'debit','checking', metadata.AP_005.testID);
            })

            await numberedSteps.start("Verify Transaction Receipt Details.", async () => {
                // await verifyReceiptDetails('AP_005_2_receipt_details_sale','0029','Failed to extract.',entryMode.tap,'150.00','SALE');
                await verifyReceiptDetails('AP_005_2_receipt_details_sale','0029',entryMode.tap,'150.00','SALE');
            })
